...
import {
  ...
  prependClipRecipeEditButton,
  revealContextualFormField
...
import { mountPodcastAudienceBilling } from "./podcast-admin-audience-billing.js";
...
function startPodcastAdmin(root) {
  ...
  const podcastAnalytics = mountPodcastAnalytics({
    ...
    setStatus,
    friendlyError,
    canEditEpisodes: () => canManageCreatives
  });
  ...
  const audienceBilling = mountPodcastAudienceBilling({
    root,
    client,
    apiOrigin,
    text: adminText,
  ...
  workspaceGroups = mountPodcastAdminWorkspaces({
    ...
    loaders: {
      ...
      subscribers: () => audienceBilling.loadSubscribers({ reset: true }),
      sponsors: loadCampaigns,
      billing: audienceBilling.loadBilling
    }
  });
  ...
  mountPodcastContextualEditing({ root });
    ...
    showSelect.addEventListener("change", async () => {
      ...
      if (audiencePanel && !audiencePanel.hidden) {
        ...
        if (workspaceGroups.isOpen("subscribers")) {
          await audienceBilling.loadSubscribers({ reset: true });
        }
      }
      ...
      if (
        ...
        await audienceBilling.loadBilling();
      }
    });
  }
  ...
  function applySavedMarketingLinkFields(row, { edit = false } = {}) {
    applyContextualMarketingLink(marketingLinkForm, row, {
      edit,
      onChange: updateMarketingLink
    });
  }
  ...
  function renderClipLibrary() {
    ...
    const cards = clipLibraryRows.map((clip) => {
      ...
      row.innerHTML = `
        ...
        ${media.container}`;
      if (canEditTranscripts && clip.episodeId) {
        prependClipRecipeEditButton(
          row.querySelector(".podcast-admin__clip-actions"), clip, adminText
        );
      }
      return row;
    });
  ...
  function selectClipRecipe(clipId) {
    ...
    fillClipRecipe(clip);
    revealContextualFormField(clipForm, "title", { block: "nearest" });
    return true;
  }
