...
current exact state is 31 passed, nine pending, one running Resend terminal-event
reconciliation, and zero failed.
...
particular, the documented Resend suppression scenario now reconciles its
signed terminal webhook by exact provider ID without retaining the recipient
or payload; the platform still cannot manufacture a pass from an accepted-only
event.
...
| Stripe | BLOCK | The read-only test-mode gate has 14 passes and one policy blocker. Launch Lab now has real provider evidence for renewal, payment failure, recovery, refund, and cancellation and is 8/11 overall. Hosted Checkout success and provider-originated duplicate/out-of-order delivery remain pending. The Store-derived Podcast subscription policy is still an explicitly unapproved staging candidate; Checkout remains disabled, and no Stripe Tax Rate has been created from unapproved facts |
...
| Full Podcast verification | Passing locally and in CI | Podcast `0.2.12`, PRs #76–#77 / merge `bacfd95ff4a4a5bdec8765c3becae593b9411a0c`, shared Platform `0.11.3` / worker-core `0.3.4`, and green CI runs `30739428881` and `30739656039` pass the 479-file tracked secret scan, zero-vulnerability audit, generated Cloudflare types, typecheck, 162 test files / 661 tests, workflow contracts, migration replay, and staging/production dry bundles. Staging migration `0085` and Worker version `8281415e-a525-4e87-b4f3-21909996cdc0` are deployed; production remains untouched and migrations `0067`–`0085` remain unapplied there |
...
| Stripe test-clock lifecycle | Five real provider outcomes complete; hosted Checkout/delivery-order evidence pending | Migration `0081` keeps newest Stripe provider state authoritative. Migrations `0084`–`0085` add a staging/test-only resumable state machine, one reusable $1 tax-free Launch Lab Product/Price, and a single resumable refund identifier through shared worker-core `0.3.4`. Protected current-source run `30739786454` completed all 22 transitions in 3m42s: the direct test subscription activated, renewed, became `past_due` with Stripe's decline-after-attachment test method, recovered with a new test method and paid invoice, resolved the exact paid PaymentIntent through Invoice Payments, completed one idempotent full refund, and canceled. Every subscription status waited for the actual provider-signed production webhook projection, while refund waited for terminal provider success; all journal rows processed with zero failures. The first protected run exposed Stripe's documented Refund shape without `livemode`, failed closed before recording evidence, and same-commit attempt 2 (`30739499000`) reused the exact refund idempotently and cleaned its fixtures. Both clocks and their customer/subscription simulations were deleted; Launch Lab checkout/source/subscription/listener rows returned to zero, foreign keys remained clean, and Stripe test mode contains zero clocks, one reusable Product, one reusable Price, and two succeeded rehearsal refunds. Stripe is 8/11 in the current ledger; hosted Checkout success and provider-originated duplicate/out-of-order delivery remain pending. Pre-migration staging Time Travel bookmark: `00000b3c-00000000-000050bb-a95cc2eeba2a49fbef1d03ff591dd714`. Production remains untouched and unmigrated |
...
| Directory and YouTube policy contracts | Five scenarios complete; unlisted uploads and real directory outcomes pending | Podcast `0.2.7` / PR #72 makes root timing explicit and reuses the production directory packet, exact canonical-feed validator, and root-publication planner. Podcast `0.2.8` / PR #73 then consumes the existing scheduled YouTube health row only when it exactly matches the configured channel, is less than 24 hours old, and has no active lease; reconciliation performs no provider call and returns no account reference. Protected run `30739786454` leaves directory packet/canonical validation at 2/5 and YouTube identity/timing/bonus exclusion at 3/5. Across all providers the exact current ledger is 31 passed / nine pending / one running Resend outcome / zero failed. Real owner verification, ingestion/recovery, and both controlled unlisted uploads remain pending; production is untouched |
...
- Idempotently reconcile Podcast test/live Product, Prices, Portal, and webhook
  configuration.
- Retain the completed real test-clock renewal, failed-renewal, recovery,
  refund, and cancellation automation. Add hosted Checkout success, Portal, and
  provider-originated duplicate/out-of-order delivery without manufacturing
  provider evidence or enabling public Checkout.
