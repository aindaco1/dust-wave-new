...
| Full Podcast verification | Passing locally; branch CI pending | Secret scan, high-severity dependency audit, Cloudflare types, typecheck, 127 test files / 514 tests, workflow lint, and staging/production dry-run bundles pass |
...
| Automatic show-notes proposals | Implemented; staging migration and first boundary pending | Migration `0071` adds private, review-only proposals keyed by the final working master, approved transcript identity/revision/digest, episode-copy digest, output language, model, and prompt version. Staging discovery prepares the transcript language and, when different, the show language with four claims per scheduler run, three attempts per fingerprint, short lease recovery, strict bounded output validation, and content-minimal system audits. Current-only reads hide stale master, transcript, or episode-copy proposals. The existing bilingual Admin editor loads a ready proposal automatically without applying it; manual regeneration is collapsed as a secondary tool. The episode, WYSIWYG, News, RSS, YouTube, media, and publication state remain unchanged until explicit apply and save. Production exits before D1 with both automation and Workers AI disabled. Worker checks pass across 127 files / 514 tests and both dry bundles; the complete site Podcast gate passes after splitting the contract to preserve the established per-module performance budget |
