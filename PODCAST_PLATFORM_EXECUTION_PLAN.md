...
Don't Settle source-test episode on 2026-08-01. It reported seven passes, three
human-review blockers, zero waits, zero failures, and clean D1 referential
integrity. The remaining blockers are the current transcript review, its exact
word-alignment quality gate, and chapter review; media and production review
now pass.
...
| Episode | BLOCK | Enhanced master revision 2, its exact delivery MP3/player peaks, and the current-audio production review are approved. Complete the current transcript review, exact word-alignment quality gate, and chapter review; then inspect the automatically proposed clip/audiogram evidence |
...
| Full Podcast verification | Passing locally and in CI | Secret scan across 403 tracked text files, zero high-severity dependency findings, Cloudflare types, typecheck, 134 test files / 551 tests, workflow lint, and staging/production dry-run bundles pass on Podcast merge `0b3d526` |
...
| Automatic transcription handoff | Complete in staging; human transcript review pending | The Worker creates source-language transcription work only after the shared final-master predicate. Podcast merge `b6d4dd8` made a retry revive a terminal dispatcher row without duplicating the source job; protected recovery run `30701049128` prepared five immutable chunks, and revision 2 completed with all five provider calls successful. Shared Platform merge `ed90dd1` adds one opt-in readable-caption policy, and Podcast merge `fc928ef` fingerprints it as `workers-ai-segment-caption-v2` for both direct and chunked paths while retaining legacy retry compatibility and excluding approved transcripts from automatic replacement. Worker version `496029bb-8f9f-465e-b8aa-7403f7f43215` created immutable revision 3 against the exact current master; protected run `30701774189` and the Queue completed all five chunks with no failures. Revision 3 has 723 cues, zero cues under 500 ms, over 10 seconds, or over 25 characters/second, zero overlaps, a 160-character maximum, unchanged first/last timing, and zero invented word timing. The Admin reports zero structural signals but correctly keeps the revision in `needs_review`; a separate private-corpus test proves that the deterministic segmentation transform preserves normalized text for identical input. Production remains untouched |
| Automatic word-alignment handoff | Complete in staging; first approved transcript pending | Worker version `4888a7bf-b621-4905-be09-81f9cd2a3240` discovers approved English/Spanish transcript revisions only after the shared final-master decision, then reuses the existing immutable projection/fingerprint/manifest/job/audit primitive and normalized processor dispatcher. Current adapter/model/settings and pinned runner digests suppress stale work; active or review-ready jobs suppress reruns; retryable failures reopen only the same job under the existing five-attempt ceiling; production exits before D1. Real-schema query preparation, system ownership, scan isolation, repeat scheduling, and the Admin break-glass path are automated. The pre-deploy content-free query found zero approved current transcripts, alignment jobs, or system alignment audits. After the first 10:20 UTC scheduled boundary, health passed and both job and system-audit counts remained zero, proving that the unapproved source transcript did not advance |
| Automatic show-notes proposals | Complete in staging; first approved transcript pending | Migration `0071` and Worker version `dc9481fa-6f82-4c85-8462-6f82a5931995` add private, review-only proposals keyed by the final working master, approved transcript identity/revision/digest, episode-copy digest, output language, model, and prompt version. Staging discovery prepares the transcript language and, when different, the show language with four claims per scheduler run, three attempts per fingerprint, short lease recovery, strict bounded output validation, and content-minimal system audits. Current-only reads hide stale master, transcript, or episode-copy proposals. The existing bilingual Admin editor loads a ready proposal automatically without applying it; manual regeneration is collapsed as a secondary tool. The episode, WYSIWYG, News, RSS, YouTube, media, and publication state remain unchanged until explicit apply and save. Production exits before D1 with both automation and Workers AI disabled. Worker checks pass across 127 files / 514 tests and both dry bundles; the complete site Podcast gate passes after splitting the contract to preserve the established per-module performance budget. The staging migration exposed 26 expected columns, zero proposal rows, and zero foreign-key errors. After the first 10:45 UTC scheduler boundary, there were still zero ready/generating/failed proposals and zero system completion/failure audits because no transcript is approved, proving the current review gate did not advance prematurely. The exact Admin commit is deployed at `https://9dda2a06.dust-wave-website-staging.pages.dev`; authenticated mock review at 1440 px and 320 px had zero horizontal overflow with both collapsed and expanded secondary controls, and the staging alias serves the exact commit with zero mobile overflow |
