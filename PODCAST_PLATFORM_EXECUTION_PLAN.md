...
| Dynamic ads | BLOCK | Refresh the current signed virtual-audio/load evidence, approve the episode plan, select a decision, and complete a qualified direct-sponsor pilot |
...
| Full Podcast verification | Passing locally and in CI | Secret scan across 423 tracked text files, zero dependency vulnerabilities at the configured audit threshold, generated Cloudflare types, typecheck, 141 test files / 585 tests, workflow contracts, and staging/production dry-run bundles pass through Podcast PR #49 / merge `22c718e` and green CI run `30710089596` |
...
| Automatic show-notes proposals | Complete in staging; v6 live content review in progress | Migration `0071` keeps every proposal private, review-only, exact-revision-bound, and unapplied. Podcast PRs #41–#44 add exact source-evidence checks for named entities and speaker labels, content-free failures, one bounded repair attempt that never reuses rejected output, and the current JSON-schema-capable `@cf/meta/llama-4-scout-17b-16e-instruct` model. The first v5 proposal reached `ready`, but direct private review found undeclared attribution prose and flat H1-heavy Markdown; it was not applied. PRs #46 and #48 therefore require empty declared attributions, neutral topic-based prose, no person/pronoun/host/guest/speaker as the subject of English or Spanish speech verbs, newline-separated H2/list structure, and no H1. The first v6 attempt also exposed that a classified safe failure was written only to audit metadata while the ledger retained its generic fallback. PR #49 fixes that shared boundary for show notes, chapters, and clips and adds a ledger-level binding test. Worker version `fbc36efe-90d4-4db8-825f-11fb578824d8` is deployed to staging; the remaining bounded retry and private content review must prove the resulting proposal before this row advances to editorial review. Episode, WYSIWYG, News, RSS, YouTube, media, and publication state remain unchanged until explicit apply and save. Production exits before D1 with all editorial AI disabled |
...
2. Verify the deployed v6 entity, attribution, and Markdown quality gate against
   the private launch transcript; keep every rejected or ready draft unapplied
   until subjective editorial review.
