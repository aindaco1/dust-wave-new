...
| Full Podcast verification | Passing locally and in CI | Podcast `0.2.18`, PRs #76–#83 / merge `5d7c82dd3d1d08552d0e91e10d8deaa1728daacd`, shared Platform `0.11.5` / worker-core `0.3.6`, and green CI run `30743081635` pass the 490-file tracked secret scan, zero-vulnerability audit, generated Cloudflare types, typecheck, 165 test files / 676 tests, workflow contracts, migration replay, and staging/production dry bundles. Staging migrations through `0088` and Worker version `acd8866c-0204-4a72-bfbf-829cd368646b` are deployed; production is untouched and migrations `0067`–`0088` remain unapplied there |
...
| Stripe provider lifecycle | Eleven provider scenarios complete in staging | Migrations `0081`, `0084`–`0087` keep newest provider state authoritative and add resumable test-clock/refund/hosted-Checkout phases plus a bounded subscription-order index. Shared worker-core `0.3.6` supplies the one Stripe Event retrieve/retry boundary. Current-source run `30743193839` replayed all 22 test-clock transitions plus the Portal configuration rehearsal in 3m48s. Hosted run `30743366702` completed all eight phases, returned Chrome to `/admin/podcasts/?checkout=launch-lab-success`, recorded signed activation and cancellation, deleted its Customer, and cleaned exact fixtures. Same-commit reconciliation `30743507706` completed in 41s and restored Stripe 11/11 plus Resend 4/4 without manufacturing or copying evidence. Checkout/source/subscription/listener rows are zero, failed Stripe journal rows are zero, and foreign keys are clean. Production remains untouched and unmigrated |
| Listener Billing Portal boundary | Runtime and provider configuration PASS; consented-listener UX pending | Podcast `0.2.17` adds stateful integration coverage around the existing DRY listener Portal route: exact authenticated Stripe Customer and Podcast-only configuration selection, exact account return URL, no-store response, CSRF and entitlement rejection before provider I/O, per-session rate limiting, malformed hosted-destination rejection, and foreign-key integrity. Podcast `0.2.18`, migration `0088`, and protected run `30743193839` add a resumable staging/test-only provider rehearsal that creates a synthetic Customer without email or payment method, re-attests its metadata, creates and validates the exact configured Portal session and return URL, never persists or returns the hosted URL, deletes the Customer, and leaves no incomplete rehearsal. Pre-`0088` Time Travel bookmark: `00000b4c-00000000-000050bb-0bf5a0a66211785876b1f9be1457df80`. A real authenticated listener browser return remains deferred until a consented fixture exists; production is untouched |
