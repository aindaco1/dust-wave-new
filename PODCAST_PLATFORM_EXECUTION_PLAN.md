...
| Automatic show-notes proposals | Complete in staging; v6 live content review in progress | Migration `0071` keeps every proposal private, review-only, exact-revision-bound, and unapplied. Podcast PRs #41–#44 add exact source-evidence checks for named entities and speaker labels, content-free failures, one bounded repair attempt that never reuses rejected output, and the current JSON-schema-capable `@cf/meta/llama-4-scout-17b-16e-instruct` model. The first v5 proposal reached `ready`, but direct private review found undeclared attribution prose and flat H1-heavy Markdown; it was not applied. PRs #46 and #48 therefore require empty declared attributions, neutral topic-based prose, no person/pronoun/host/guest/speaker as the subject of English or Spanish speech verbs, newline-separated H2/list structure, and no H1. Failures persist only bounded content-free codes. Worker version `7ad3d053-05ac-44fd-b8a9-2fde9028308e` is deployed to staging with prompt `show-notes-v6-grounded-quality`; the scheduler and private content review must prove the resulting proposal before this row advances to editorial review. Episode, WYSIWYG, News, RSS, YouTube, media, and publication state remain unchanged until explicit apply and save. Production exits before D1 with all editorial AI disabled |
