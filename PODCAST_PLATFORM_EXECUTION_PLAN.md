...
current exact state is 30 passed, 10 pending, one running Resend terminal-event
...
| Stripe | BLOCK | The read-only test-mode gate has 14 passes and one policy blocker. Launch Lab now has real provider evidence for renewal, payment failure, recovery, and cancellation and is 7/11 overall. Hosted Checkout success, refund, and provider-originated duplicate/out-of-order delivery remain pending. The Store-derived Podcast subscription policy is still an explicitly unapproved staging candidate; Checkout remains disabled, and no Stripe Tax Rate has been created from unapproved facts |
...
| Full Podcast verification | Passing locally and in CI | Podcast `0.2.10`, PR #75 / merge `4dc2734e09855072acd89b832651020274891472`, shared Platform `0.11.2` / worker-core `0.3.3`, and green CI run `30738923555` pass the 474-file tracked secret scan, zero-vulnerability audit, generated Cloudflare types, typecheck, 162 test files / 661 tests, workflow contracts, migration replay, and staging/production dry bundles. Staging migration `0084` and Worker version `9c247c34-50ae-4327-968b-a5510442f395` are deployed; production remains untouched and migration `0084` remains unapplied there |
...
| Stripe test-clock lifecycle | Four real provider outcomes complete; hosted Checkout/refund/delivery-order evidence pending | Migration `0081` keeps newest Stripe provider state authoritative. Migration `0084` adds a staging/test-only resumable state machine and one reusable $1 tax-free Launch Lab Product/Price through shared worker-core `0.3.3`. Protected run `30739014132` completed all 21 phases in 3m45s: the direct test subscription activated, renewed, became `past_due` with Stripe's decline-after-attachment test method, recovered with a new test method and paid invoice, and canceled. Every status waited for the actual provider-signed production webhook projection; all journal rows processed with zero failures. The clock and its customer/subscription were deleted, Launch Lab checkout/source/subscription/listener rows returned to zero, foreign keys remained clean, and only one reusable test Product/Price remains. Stripe is 7/11 in the current ledger; hosted Checkout success, refund, and provider-originated duplicate/out-of-order delivery remain pending. Pre-migration staging Time Travel bookmark: `00000b35-00000046-000050bb-79b74ee9145a951e4f70ac7e450118c6`. Production remains untouched and unmigrated |
...
| Directory and YouTube policy contracts | Five scenarios complete; unlisted uploads and real directory outcomes pending | Podcast `0.2.7` / PR #72 makes root timing explicit and reuses the production directory packet, exact canonical-feed validator, and root-publication planner. Podcast `0.2.8` / PR #73 then consumes the existing scheduled YouTube health row only when it exactly matches the configured channel, is less than 24 hours old, and has no active lease; reconciliation performs no provider call and returns no account reference. Protected run `30739014132` leaves directory packet/canonical validation at 2/5 and YouTube identity/timing/bonus exclusion at 3/5. Across all providers the exact current ledger is 30 passed / 10 pending / one running Resend outcome / zero failed. Real owner verification, ingestion/recovery, and both controlled unlisted uploads remain pending; production is untouched |
...
3. Continue autonomous contract and recovery tests for hosted Stripe Checkout,
   refunds and provider-originated replay/order behavior now that the real
   test-clock renewal/failure/recovery/cancellation matrix is complete. Keep
...
- Idempotently reconcile Podcast test/live Product, Prices, Portal, and webhook
  configuration.
- Retain the completed real test-clock renewal, failed-renewal, recovery, and
  cancellation automation. Add hosted Checkout success, refund, Portal, and
