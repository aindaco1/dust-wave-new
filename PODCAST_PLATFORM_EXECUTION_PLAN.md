...
current exact state is 27 passed, 14 pending, zero running, and zero failed.
...
| Full Podcast verification | Passing locally and in CI | Podcast `0.2.9`, PR #74 / merge `ff1fd38079ae247cc94dc048d9151e82137239b1`, and green CI run `30738267721` pass the 474-file tracked secret scan, zero-vulnerability audit, generated Cloudflare types, typecheck, 161 test files / 659 tests, workflow contracts, migration replay, and staging/production dry bundles. Staging migration `0083` and Worker version `2e74413c-d3b8-4f0b-a885-6a5d7b465f12` are deployed; production remains untouched and unmigrated |
...
| Directory and YouTube policy contracts | Five scenarios complete; unlisted uploads and real directory outcomes pending | Podcast `0.2.7` / PR #72 makes root timing explicit and reuses the production directory packet, exact canonical-feed validator, and root-publication planner. Podcast `0.2.8` / PR #73 then consumes the existing scheduled YouTube health row only when it exactly matches the configured channel, is less than 24 hours old, and has no active lease; reconciliation performs no provider call and returns no account reference. Protected same-commit rehearsal `30738347359` attempt 2 on staging Worker `2e74413c-d3b8-4f0b-a885-6a5d7b465f12` leaves directory packet/canonical validation at 2/5 and YouTube identity/timing/bonus exclusion at 3/5. The exact ledger is 27 passed / 14 pending / zero running / zero failed. Real owner verification, ingestion/recovery, and both controlled unlisted uploads remain pending; production is untouched |
