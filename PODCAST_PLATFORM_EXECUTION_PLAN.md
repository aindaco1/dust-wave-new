...
current exact state is 35 passed, six pending, zero running, and zero failed.
...
provider reconciliation can only advance an allowlisted lifecycle state.
Stripe is 11/11 after real hosted Checkout, test-clock renewal, failure,
recovery, refund, cancellation, and provider-originated duplicate and
older-event delivery. Resend is 4/4 after same-commit reconciliation reused its
idempotency key and consumed the signed terminal suppression state without
retaining the recipient or payload.
...
| Stripe | Provider lifecycle PASS; policy promotion BLOCK | Launch Lab is 11/11 with real test-mode evidence for hosted Checkout, renewal, payment failure, recovery, refund, cancellation, duplicate delivery, and older-event reconciliation. The Store-derived Podcast subscription policy remains an explicitly unapproved staging candidate; public Checkout stays disabled, and no Stripe Tax Rate is created from unapproved facts |
...
| Resend | PASS | Current-source accepted, delivered, bounced, complained, and terminal suppression evidence reconciles through signed provider events without retaining recipient data |
...
| Full Podcast verification | Passing locally and in CI | Podcast `0.2.17`, PRs #76–#82 / merge `51cfec7654e2e62fe9f14554caef2d85978fcd8d`, shared Platform `0.11.5` / worker-core `0.3.6`, and green CI run `30742753995` pass the 488-file tracked secret scan, zero-vulnerability audit, generated Cloudflare types, typecheck, 164 test files / 670 tests, workflow contracts, migration replay, and staging/production dry bundles. Staging migrations through `0087` and Worker version `cb290269-b231-43fe-a66b-9d11939a9fe1` remain deployed; production is untouched and migrations `0067`–`0087` remain unapplied there |
| Staging Launch Lab | Worker and bilingual Admin complete in staging | One staging-only, Super-admin-only, private/no-store endpoint derives a fixed 41-scenario ledger from immutable fixture evidence and exact provider reconciliation without writing on reads. Normal show listings and billing readiness exclude every fixture row. Site commits `7fe2fb1` and `e604f46` add the protected `$1` Checkout handoff through the existing Settings panel without persisting or rendering the ephemeral URL. The Admin reuses existing metric, disclosure, spacing, i18n, CSRF, and authorization primitives rather than adding a seventh workspace. Unit, route-boundary, privacy, no-write, no-resend, locale-parity, performance-budget, desktop/mobile overflow, and authenticated browser contracts pass. Production is untouched |
| Stripe provider lifecycle | Eleven provider scenarios complete in staging | Migrations `0081`, `0084`–`0087` keep newest provider state authoritative and add resumable test-clock/refund/hosted-Checkout phases plus a bounded subscription-order index. Shared worker-core `0.3.6` supplies the one Stripe Event retrieve/retry boundary. Protected current-source run `30742231496` completed all 22 test-clock transitions in 4m02s: activation, renewal, decline-driven `past_due`, recovery, exact Invoice Payment resolution, full refund, and cancellation all waited for real provider state and signed webhook projections. Before refund, Stripe re-delivered the exact current `active` event and then an older `past_due` event to the exact staging endpoint; the signed callbacks recorded `idempotent` and `reconciled` only while the current source remained active. Hosted run `30742371116` completed all eight phases, returned Chrome to `/admin/podcasts/?checkout=launch-lab-success`, recorded activation and cancellation, deleted its Customer, and cleaned exact fixtures. Stripe is 11/11; checkout/source/subscription/listener rows are zero, failed Stripe journal rows are zero, and foreign keys are clean. Pre-`0087` Time Travel bookmark: `00000b47-00000000-000050bb-b760c14646ad58fce867b34d1d2b4224`. Production remains untouched and unmigrated |
| Listener Billing Portal boundary | Runtime contract covered; protected hosted return pending | Podcast `0.2.17` adds stateful integration coverage around the existing DRY listener Portal route: exact authenticated Stripe Customer and Podcast-only configuration selection, exact account return URL, no-store response, CSRF and entitlement rejection before provider I/O, per-session rate limiting, malformed hosted-destination rejection, and foreign-key integrity. No runtime or provider configuration changed. A protected hosted Portal visit and exact return remain a staging browser rehearsal, not manufactured evidence |
...
| Resend ordered-event reconciliation | Four provider scenarios complete in staging | Podcast `0.2.5` / PR #70 models Resend's documented complaint sequence, keeping delivery intermediate until complaint/suppression. Podcast `0.2.9` / PR #74 adds an indexed exact-provider lookup in the existing signed, content-free webhook journal for the special non-labelable suppression address, with terminal suppression precedence, provider-ID isolation, no generic-bounce inference, and bounded read-only provider retrieval only as fallback. Protected same-commit rerun `30738347359` attempt 2 leaves delivered, bounced, complained, and suppressed all passed without resending or exposing recipient/provider payload evidence. Pre-migration staging Time Travel bookmark: `00000b35-00000000-000050bb-0c13a8f0e8d4a35a3b82de64281934d0`. Production remains untouched and unmigrated |
| RSS delivery contract | Four synthetic scenarios complete; publishable item still absent | Podcast `0.2.6`, PR #71, and protected same-commit rerun `30738347359` attempt 2 execute the production public feed/media route contract for a body-free enclosure `HEAD`, streamed byte range, and approved-only Podcasting 2.0 transcript/chapter declarations. Transcript digest tampering removes only the affected declaration, fixture feeds remain directory-blocked, and the fixture remains publicly hidden. RSS is 4/4, with no publishable episode or production mutation |
| Directory and YouTube policy contracts | Five scenarios complete; unlisted uploads and real directory outcomes pending | Podcast `0.2.7` / PR #72 makes root timing explicit and reuses the production directory packet, exact canonical-feed validator, and root-publication planner. Podcast `0.2.8` / PR #73 consumes the existing scheduled YouTube health row only when it exactly matches the configured channel, is less than 24 hours old, and has no active lease; reconciliation performs no provider call and returns no account reference. Current-source runs `30742231496`, `30742371116`, and idempotent reconciliation `30742450423` leave the full matrix at 35 passed / six pending / zero running / zero failed. The six truthful holds are one native-client qualified ad request, three real directory outcomes, and two controlled unlisted uploads; production is untouched |
...
3. Continue autonomous contract and recovery tests for hosted Stripe Checkout
   and provider-originated replay/order behavior now that the real test-clock
   renewal/failure/recovery/refund/cancellation matrix is complete. Keep
   extending tax-core, Pool grants, YouTube resumable uploads, Resend lifecycle
   events, directories, and ads. Feed content-free evidence into the existing
   ledger; do not create provider-specific readiness stores.
...
- Retain the completed 11/11 real-provider Launch Lab: hosted Checkout,
  ...
  reconciliation. Complete a protected Customer Portal return rehearsal
  without enabling public Checkout.
